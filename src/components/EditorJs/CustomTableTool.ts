...
class TableTool {
  ...
  private createContextMenu() {
    ...
    if (this.table) {
      ...
      const tableRect = this.table.getBoundingClientRect();
      const topPosition = tableRect.top;
      const menuHeight = 40;

      if (topPosition < 50) {
        menu.style.top = `${this.table.offsetHeight + 5}px`;
        menu.style.left = "0px";
  ...
  private attachEventListeners() {
    ...
    this.table.addEventListener("contextmenu", (e: any) => {
      ...
      if (topPosition < 50) {
        ...
        this.contextMenu.style.top = `-${this.contextMenu.offsetHeight + 45}px`;
        this.contextMenu.style.width = "100%";
      } else {
        this.contextMenu.style.position = "absolute";
        this.contextMenu.style.left = "0px";
        this.contextMenu.style.bottom = `-${this.contextMenu.offsetHeight + 45}px`;
        this.contextMenu.style.width = "100%";
      }
