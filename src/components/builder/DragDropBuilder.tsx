...
export default function DragDropBuilder({ elements, onElementsChange, pageLayouts, activeView }: DragDropBuilderProps) {
        ...
        const deleteNested = (els: EmailElement[]): EmailElement[] => {
            return els
                ...
                .map(el => {
                    ...
                    if (el.columns) {
                        return { ...el, columns: el.columns.map(col => deleteNested(col)) };
                    }
                    return el;
                });
        };
    ...
    const addElementToContainer = (containerId: string, newElement: EmailElement) => {
        const addToContainer = (els: EmailElement[]): EmailElement[] => {
            return els.map(el => {
                ...
                if (el.columns) {
                    return { ...el, columns: el.columns.map(col => addToContainer(col)) };
                }
                return el;
            });
        };
    ...
    const addElementToColumn = (containerId: string, columnIndex: number, newElement: EmailElement) => {
    ...
    const findElementById = (els: EmailElement[], id: string): EmailElement | null => {
        for (const el of els) {
    ...
    const moveElement = (direction: 'up' | 'down') => {
        ...
        const findElementIndex = (els: EmailElement[], id: string, path: number[] = []): number[] | null => {
            for (let i = 0; i < els.length; i++) {
                ...
                if (els[i].children) {
                    const found = findElementIndex(els[i].children!, id, [...path, i, -1]);
                    if (found) return found;
                }
                if (els[i].columns) {
                    for (let colIdx = 0; colIdx < els[i].columns!.length; colIdx++) {
                        const found = findElementIndex(els[i].columns![colIdx], id, [...path, i, -2, colIdx]);
                        if (found) return found;
                    }
                }
            }
        ...
        const updateElements = (els: EmailElement[], path: number[], newElements: EmailElement[]): EmailElement[] => {
            if (path.length === 1) {
                const newEls = [...els];
            ...
            return els.map((el, idx) => {
                if (idx === path[0]) {
                    if (path[1] === -1 && el.children) {
                        return { ...el, children: updateElements(el.children, path.slice(2), newElements) };
                    } else if (path[1] === -2 && el.columns) {
                        const colIdx = path[2];
                        const newColumns = [...el.columns];
                        newColumns[colIdx] = updateElements(newColumns[colIdx], path.slice(3), newElements);
                        return { ...el, columns: newColumns };
                    }
                }
                return el;
